...
export default async (request: Request, context: Context) => {
  ...
  const isAsset = /\.(js|css|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|eot|json|xml|webmanifest|map)$/i.test(url.pathname);
  ...
  if (!isAsset && !isInternalRequest && !isDeno && !isWellKnown && !isManifest) {
